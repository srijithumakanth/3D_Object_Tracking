...
int main(int argc, const char *argv[])
{
    ...
    vector<string> Detectors={"FAST", "BRISK", "ORB", "SHITOMASI"};//{"SHITOMASI", "HARRIS" ,"SIFT", "FAST", "BRISK", "ORB", "AKAZE"};
    vector<string> Descriptors={"BRIEF", "ORB", "BRISK", "SIFT", "FREAK", "AKAZE"};
    ...
    for(auto detType: Detectors) //Loop Over Detector
    {
        for(auto descType: Descriptors) //Loop Over Descritpor
        {
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                // optional : only keep keypoints on the preceding vehicle
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    // std::cout << "Matched descriptors size: " << matches.size() << std::endl;
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        {
                            std::cout << "Computing TTC " << std::endl;
                            //// STUDENT ASSIGNMENT
                            //// TASK FP.2 -> compute time-to-collision based on Lidar data (implement -> computeTTCLidar)
                            double ttcLidar; 
                            computeTTCLidar(prevBB->lidarPoints, currBB->lidarPoints, sensorFrameRate, ttcLidar);
                            
                            std::cout << "TTC LiDAR is: " << ttcLidar << std::endl;
                            ...
                            std::cout << "TTC Camera is: " << ttcCamera << std::endl;
